...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      ...
      try {
        const response = await fetch(`${nestJsApiUrl}/users/profile`, { // Assuming a /users/profile endpoint
          method: 'GET',
        ...
        if (response.ok) {
          ...
          console.log('Unauthorized access to dashboard, redirecting to signin.');
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-blue-500 to-purple-600 text-white p-4">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl text-center text-gray-800">
        ...
        {userEmail ? (
          <p className="text-lg mb-6">Hello, <span className="font-semibold text-purple-600">{userEmail}</span>!</p>
        ) : (
          <p className="text-lg mb-6">Loading user data...</p>
        )}
        <p className="text-md mb-8">This is your protected area, accessible only after successful authentication.</p>
