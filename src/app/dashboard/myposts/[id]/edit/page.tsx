...
async function getPostData(postId: number): Promise<Post> {
  ...
  try {
    const response = await fetch(`${nestJsApiUrl}/posts/${postId}`, fetchOptions);
    ...
    if (response.ok) {
      ...
      console.error(`Authentication error fetching post ${postId}. Status: ${response.status}. Redirecting.`);
      redirect('/auth/signin');
    } else {
    ...
    throw new Error(err.message || `An unexpected error occurred while fetching post ${postId}.`);
  }
}
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-white to-gray-100 px-4 py-8">
        <div className="text-center p-8 bg-white rounded-2xl shadow-lg">
          <h2 className="text-2xl font-bold text-red-600 mb-4">Error Loading Post for Editing</h2>
          <p className="text-gray-700 mb-6">{error || "Post data could not be retrieved."}</p>
    ...
    <main className="min-h-screen bg-gradient-to-br from-white to-gray-100 px-4 py-8 flex items-center justify-center">
      <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-lg">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Edit Post: "{post.title}"</h1>
